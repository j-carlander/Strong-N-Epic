...
export function WorkoutPage(): JSX.Element {
  ...
  const filteredWorkouts = workouts.filter(
    (workout) =>
      chosenDate.toDateString() === new Date(workout.startTime).toDateString()
  );
  const bookedWorkouts = workouts.filter((workout) =>
    currentUser.details.bookedWorkouts.includes(workout._id)
  );
